...
class SEOManager {
  ...
  update({ title, description, route, image, type }) {
    ...
    const desc =
      description ||
      "A comprehensive learning platform for Docker and containerization.";
    const cleanRoute =
      route && route !== "overview" ? String(route).replace(/^\/+/, "") : "";
    const pageUrl = cleanRoute ? `${DOMAIN}/${cleanRoute}` : `${DOMAIN}/`;
